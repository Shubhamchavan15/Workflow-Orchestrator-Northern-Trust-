...
const StatusIcon = ({ status }) => {
  ...
  return <FaClock className="text-gray-300 dark:text-gray-600 text-xl" />;
};
    ...
    <div className="max-w-3xl mx-auto px-6 py-10">
      ...
      <form onSubmit={handleTrack} className="flex gap-3 mb-8">
        <input value={executionId} onChange={e => setExecutionId(e.target.value)}
          ...
          className="flex-1 px-5 py-3 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-800 dark:text-white rounded-xl outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all" />
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-300 rounded-xl px-5 py-4 mb-6 text-sm">
          {error}
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-lg p-6 transition-colors duration-300">
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Execution ID</p>
                <p className="text-xl font-bold text-gray-800 dark:text-white">{data.execution_id}</p>
              </div>
              <span className={`px-4 py-2 rounded-full text-sm font-bold ${statusColor(data.status)}`}>{data.status}</span>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 dark:text-gray-300">
              <p><span className="font-semibold text-gray-700 dark:text-gray-100">Order ID:</span> {data.order_id}</p>
              <p><span className="font-semibold text-gray-700 dark:text-gray-100">Customer:</span> {data.customer_name}</p>
              <p><span className="font-semibold text-gray-700 dark:text-gray-100">Amount:</span> ₹{data.amount?.toLocaleString()}</p>
              <p><span className="font-semibold text-gray-700 dark:text-gray-100">Started:</span> {data.created_at?.slice(0, 19).replace("T", " ")}</p>
            </div>
          </div>

          {/* Task progress */}
          <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-lg p-6 transition-colors duration-300">
            <h2 className="text-lg font-bold text-gray-700 dark:text-white mb-5">Workflow Progress</h2>
              ...
              {taskKeys.map((key, idx) => {
                ...
                return (
                  <div key={key} className="flex items-center gap-4">
                    ...
                    <div className="flex-1 flex items-center justify-between bg-gray-50 dark:bg-gray-700 rounded-xl px-4 py-3">
                      <span className="font-semibold text-gray-700 dark:text-gray-200 text-sm">
                        {idx + 1}. {TASK_LABELS[key]}
                      </span>
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${statusColor(state)}`}>{state}</span>
                    </div>
                  </div>
                );
              })}
          ...
          {logs.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-lg p-6 transition-colors duration-300">
              <h2 className="text-lg font-bold text-gray-700 dark:text-white mb-4">Execution Logs</h2>
